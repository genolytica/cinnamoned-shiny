...
#
# TODO: -Find a way to get the retention time deviation after retcor so as to be
#        able to automatically select the best bw
#       -Extend the functionality of the script to perform single file analysis. 
#        In this case, no annotation will be performed, only peak finding and 
#        the return will be the peak matrix from the function peakTable. The 
#        sample info file should be treated similarly. Try to make use of 
#        mclapply. 
#
...
xcmsPipeline <- function(path.to.raw,info.file,param.file,path.to.trunc=NULL,
    ...
    if (multicore) {
        if (!require(parallel)) {
            warning("R package parallel not present... Switching to single ",
                "core...",call.=FALSE)
            multicore <- FALSE
        }
        else {
    ...
    if (missing(param.file)) {
        message("Parameters file missing! The default parameters will ",
            "be used...")
        ...
        if (f %in% c("yaml","yml")) {
            ...
            error=function(e) {
                message("Bad parameter file! Defaults will be used...")
    ...
    if (persample) {
        if (multicore) {
            ...
            if (annotate) {
                ...
                peak.tab <- mclapply(xset.raw,peakTable,method="maxint",
                    value="into",intensity="into")
            for (i in 1:length(peak.tab))
                peak.tab[[i]] <- as.data.frame(peak.tab[[i]])
        }
        else { # parallel not present or single core machine
            ...
            for (i in 1:length(the.raw.files)) {
                ...
                if (annotate) {
                    ...
                    peak.tab[[i]] <- as.data.frame(peakTable(xset.raw[[i]],
                        method="maxint",value="into",intensity="into"))
            }
        }
        peaks <- peak.tab
        ...
        pd <- as.data.frame(as.character(meta.data$Class),
            row.names=as.character(meta.data$Replicate))
        names(pd) <- c("Class")
        ...
        xset.group.fi <- fillPeaks(xset.group[[length(param.list$group$bw)]],
            method="chrom")
        ...
        if (annotate) {
            ...
            peaks <- as.data.frame(peakTable(xset.group.fi,method="maxint",
                value="into",intensity="into"))
        }
    }
...
read.verify.yaml <- function(param.file) {
    ...
    for (n in names(pars)) {
        ...
        if (!all(valid)) {
            invalid <- which(!valid)
            for (i in invalid)
                message(paste("Unknown",n,"parameter :",pars[[n]][[i]],
                    "Ignoring..."))
...
read.verify.custom <- function(param.file) {
    ...
    pars.table <- read.delim(param.file,header=FALSE,comment.char="#",
        row.names=NULL,colClasses="character",col.names=c("Parameter","Value"))
    ...
    nams <- list(
        pars.table$Parameter[(b+1):(r-1)],pars.table$Parameter[(r+1):(f-1)],
        pars.table$Parameter[(r+1):(f-1)],pars.table$Parameter[(f+1):(g-1)],
        pars.table$Parameter[(g+1):(t-1)],pars.table$Parameter[(t+1):(e-1)],
        pars.table$Parameter[(e+1):(a-1)],
            pars.table$Parameter[(a+1):nrow(pars.table)]
    )
    vals <- list(
        parse.list.members(as.list(pars.table$Value[(b+1):(r-1)])),
    ...
    for (n in names(nams)) {
        ...
        if (!all(valid)) {
            invalid <- which(!valid)
            for (i in invalid)
                message(paste("Unknown",n,"parameter :",nams[[n]][[i]],
                    "Ignoring..."))
...
static.names <- function() {
    accept.param <- list(
        ...
        c("method","fwhm","sigma","max","step","steps",
            ...
            "snthresh","sleep"),
        c("method","bw","mzwid","max","mzppm","mzabs","mzVsRTbalance","mzCheck",
            "rtCheck","kNN","minfrac","minsamp","sleep"),
        ...
        c("group","iso.flow","sigma","perfwhm","pval","cor_eic_th",
            "find.adducts","maxiso","mzabs.add","mzabs.fiso","mzabs.diso",
            "ppm.fiso","ppm.diso","polarity","rulefile","filter.valid",
            "filter.score","peak.val","filter.dbe","write.output","run.par",
            "more.adducts","fail.rules","export.what")
    )
    names(accept.param) <- c("filter","read","find","group","retcor","extract",
        "annotate")
    return(accept.param)
}
...
read.info <- function(info.file) {
    ...
    # Read information file and construct a consistent correspondence between 
    # filenames and sample info
