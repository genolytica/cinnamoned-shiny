# TODO: All should become ggplot and/or interactive?

# This file will contain several diagnostic plots

# name : vector of names to be used
# y.lim : how to construct y-axis limits? "default", "auto" or a 2-length 
#         vector, defaults to "default" (internal R plotting system is taking 
#         care of this)
# output : output plotting device (pdf, png etc.), defaults to x11 and if 
#          "same", no device will be called so that user can plot many bopxlots 
#           using par externally. In this case the second matrix (tam, if given)
#           will be disarded
# ... further arguments to boxplot
boxplot.mat <- function(mat,tam,name=NULL,log.it="auto",y.lim="default",
    output="x11",fil=NULL,...) {
...
# output : output plotting device (pdf, png etc.), defaults to x11 and currently 
#          not used
# ... further arguments to corrplot of package corrplot (pretty cool package!)
#     or to levelplot of package lattice
image.cor <- function(mat,type="pearson",effect="fancy",output="x11",
    log.mat=FALSE,fil=NULL,...) {
    ...
    if (effect=="fancy")
        ...
        levelplot(m,col.regions=cm.colors(100)[100:1],
...
# type : correlation calculation given to function cor
# effect : "simple" for a simple pairwise scatterplot, "fancy" (default) for 
#          fancier one and "large" for a summarized color scatterplot pairs
# output : output plotting device (pdf, png etc.), defaults to x11 and currently
#          not used
# ... further arguments to cpairs of package gclus or to pairs of package 
#     grDevices
scatter.cor <- function(mat,type="pearson",effect="fancy",output="x11",
    log.mat=FALSE,log.plot=FALSE,fil=NULL,...) {
...
norm.diag.plot <- function(x,y,lx=NULL,ly=NULL,iset,xn=NULL,yn=NULL,
    type="normal",log.it="auto",output="x11",fil=NULL,...) {
    openGraphics(output,fil)
...
norm.diag.pairs <- function(x,iset,l=NULL,log.it="NYI",output="x11",
    fil=NULL,...) {
    n <- ncol(x)
    ...
    # Setup the grid
    par(mfrow=c(n,n),mar=c(1,1,1,1),oma=c(1,1,0,0),mgp=c(2,0.5,0),cex.axis=0.7,
        cex.lab=0.7)
            ...
            if (i==j && i!=n && j!=n) {
                ...
                if (is.list(iset)) {
                    for (k in 1:length(iset))
                        points(x[iset[[k]],i],x[iset[[k]],j],pch=20,cex=0.5,
                            col=cols[k],...)
                }
                else
                ...
                # Meaningless to plot the possible normalization curve, just the
                # iset
                if (is.list(iset)) {
                    for (k in 1:length(iset))
                        points((x[iset[[k]],i]+x[iset[[k]],j])/2,
                            x[iset[[k]],j]-x[iset[[k]],i],pch=20,cex=0.5,
                            col=cols[k],...)
                }
                else
                    points((x[iset,i]+x[iset,j])/2,x[iset,j]-x[iset,i],pch=20,
                        col="green",...)
                grid()
            }
            else if (i==n && j==n) {
                ...
                text(c(5,5),c(7,3),c(paste("#Points:",np),paste("#I.Set:",ni)),
                    cex=1.5,font=2)
            }
        }
    }
...
# This should be used in the case of normalizing against a baseline and iset 
# must be only a list
norm.diag.base <- function(x,b,iset,l=NULL,log.it="NYI",output="x11",
    fil=NULL,...) {
    if (missing(b))
    ...
    par(mfrow=c(grid.size[1],grid.size[2]),mar=c(1,1,1,1),oma=c(1,1,0,0),
        mgp=c(2,0.5,0),cex.axis=0.7,cex.lab=0.7)
...
hist.zeros <- function(X,Y=NULL,what=c("each","total"),na=FALSE,output="x11",
    fil=NULL) { 
    what <- what[1]
    ...
    if (type=="single") {
        ...
        if (what=="each") {
            ...
            par(mfrow=c(2,1))
            barplot(zero.vec.X,col="darkred",border="yellow",las=2,
                ylab="Percentage of 0's")
            ...
            barplot(c(zero.vec.X,zero.vec.Y),col="darkred",border="yellow",
                las=2,ylab="Percentage of 0's")
            grid(nx=NA,ny=NULL)
        }
    }
...
plot.mzrt <- function(rt,mz,inten=NULL,iset=NULL,output="x11",fil=NULL,...) {
    if (!is.null(inten)) {
        ...
        jet.fun <- colorRamp(c("#00007F","blue","#007FFF","cyan","#7FFF7F",
            "yellow","#FF7F00","red","#7F0000"))
        my.jet <- round(jet.fun(ref))
        class(my.jet) <- "hexmode"
        my.hex <- paste("#",apply(as.character(my.jet),1,paste,collapse=""),
            sep="")
    }
    else {
        log.scaled <- FALSE
    ...
    par(mar=c(4,4,4,4),oma=c(0.5,0.5,0,0),cex.lab=1.2,cex.axis=0.8,font.lab=2,
        bg="lightgrey")
    plot(rt,mz,pch=20,col=my.hex,main="Peak data",xlab="Retention Time (sec)",
        ylab="m/z",xaxt="n",yaxt="n",...)
...
plot.rtdev <- function(x,y,l=NULL,lx=NULL,ly=NULL,lcl=NULL,exclude=NULL,
    log.it=FALSE,output="x11",fil=NULL,...) {
    if (is.matrix(x) || is.data.frame(x)) { # Assuming pairs of unnormalized-normalized values
        ...
        if (ncol(x)!=2 && ncol(y)!=2)
            stop("Both x and y must have two columns, 1st with raw times and ",
                "2nd with normalized!")
        two <- TRUE
    }
    else
    ...
    if (is.null(lcl) && ((is.null(lx) && !is.null(ly)) 
        || (is.null(ly) && !is.null(lx)))) {
        warning("Both lx and ly must be provided! Ignoring...",immediate.=TRUE)
        lx <- ly <- NULL
    }
    if (!is.null(lcl) && (!is.null(lx) || !is.null(ly))) {
        warning("Only one of lcl or the (lx,ly) pair can be provided! ",
            "Ignoring (lx,ly)...",immediate.=TRUE)
        lx <- ly <- NULL
    }
    ...
    if (two) {
        ...
        if (!is.null(exclude)) {
            ...
            plot(m[,1],d[,1],pch=20,col="blue",cex=0.5,
                main="Retention time deviation - raw",xlab="Reference RT (sec)",
                ylab="RT deviation from reference",ylim=ylim)
        }
        else
            plot(m[,1],d[,1],pch=20,col="blue",cex=0.5,
                main="Retention time deviation - raw",xlab="Reference RT (sec)",
                ylab="RT deviation from reference")
        if (!is.null(l))
        ...
        if (!is.null(exclude)) {
            ...
            plot(m[,2],d[,2],pch=20,col="blue",cex=0.5,
                main="Retention time deviation - normalized",
...
# l : a set of indices
# lx, ly: set of points (e.g. a curve)
plot.intma <- function(x,y,l=NULL,lx=NULL,ly=NULL,lcl=NULL,log.it=FALSE,
    output="x11",fil=NULL,...) {
    # Assuming pairs of unnormalized-normalized values
    ...
    if (two) {
        par(mfrow=c(2,1),mar=c(3,3,3,3),oma=c(0.5,0.5,0,0),mgp=c(2,0.5,0),
            cex.axis=0.7,cex.lab=0.8,cex.main=0.9)
        # Plot un-normalized
        plot(a[,1],m[,1],pch=20,col="blue",cex=0.5,
            main="Intensity mean-difference - raw",
        ...
        legend(x="topright",legend=legt,col=legc,pch=legp,cex=0.7,bg="white")
        # Plot normalized
        plot(a[,2],m[,2],pch=20,col="blue",cex=0.5,
            main="Intensity mean-difference - normalized",
            xlab="Mean intensity (with reference)",ylab="Intensity difference")
        if (!is.null(l))
...
plot.match <- function(rtr,mzr,rtn,mzn,rto=NULL,mzo=NULL,output="x11",
    fil=NULL,...) {
    ...
    if (two) {
        par(mfrow=c(2,1),mar=c(3,3,3,3),oma=c(0.5,0.5,0,0),cex.lab=0.9,
            cex.main=1,font.lab=2,bg="lightgrey")
        # Before alignment
        plot(rtr,mzr,col="yellow",main="Peak data before alignment",
            xlab="Retention Time (sec)",ylab="m/z",xaxt="n",yaxt="n",...)
        ...
        abline(v=gt,lty=3,col="lightgrey")
        # After alignment
        plot(rtr,mzr,col="yellow",main="Peak data after alignment",
            xlab="Retention Time (sec)",ylab="m/z",xaxt="n",yaxt="n",...)
        rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],
            col="darkblue")
        points(rtr,mzr,col="yellow",cex=1)
        ...
        par(mar=c(4,4,4,4),oma=c(0.5,0.5,0,0),cex.lab=1.2,cex.axis=0.8,
            font.lab=2,bg="lightgrey")
        plot(rtr,mzr,col="yellow",main="Peak data",xlab="Retention Time (sec)",
            ylab="m/z",xaxt="n",yaxt="n",...)
        rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],
            col="darkblue")
        points(rtr,mzr,col="yellow",cex=1)
...
plot.rvn <- function(x,y,sx=NULL,sy=NULL,lim=NULL,output="x11",fil=NULL,...) {
    ...
    par(mfcol=c(1,2),mar=c(4,4,4,4),oma=c(1,1,0,0),pch=20,cex=0.8,cex.lab=1.3,
        cex.axis=1.2,cex.main=1.4,font.lab=2)
    plot(x,y,pch=20,col="darkgreen",xlab="Reference intensity (log2)",
        ylab="Raw intensity (log2)",main=main.xy)
    ...
    grid()
    plot(a,m,pch=20,col="blue",xlab="0.5*(Reference + Raw) (log2)",
        ylab="Raw - Reference (log2)",main=main.md)
    if (!is.null(sx)) # No need to check both, it has been done above
