# An R program to annotate the peaks found with xcms given a resulting xcmsSet 
# object and several parameters controling mostly the tolerance in mass 
# deviation for the isotope decomposition and the formula generation as well as 
# the isotope cluster formation
#
...
# group        : CAMERA peak grouping to calculate isotope clusters, can be 
#                "fwhm", "corr" or "both". It defaults to "both". If one of the 
#                other is given, the process is deviated (see Note) but faster
# iso.flow     : The way to calculate isotopic distributions (using only FWHM 
#                grouping or both FWHM and corr) as the ordering becomes not 
#                relevant if the isotopes are calculated using only one method. 
#                Can be "fwhm" (default) or "both"
...
# mzabs.add    : Absolute mass difference for tolerance in adduct finding. 
#                Defaults to 0.01
# mzabs.fiso   : Absolute mass difference for tolerance in isotope finding. 
#                Defaults to 0.01
# mzabs.diso   : Absolute mass difference for tolerance in isotope decomposing 
#                for formula calculation. Defaults to 0.001
# ppm.fiso     : Same as mzabs.fiso, but in ppm and the two are added
# ppm.diso     : Same as mzabs.diso, but in ppm and the two are added
# polarity     : ESI polarity of MS machine. Defaults to "positive" and is only 
#                used if group="corr" and/or find.adducts=TRUE
# rulefile     : File containing rules for adducts... If NULL, the default from
#                package CAMERA will be used
# peak.val     : What value to use as intensity of peak when calculating 
#                isotopes. Defaults to peak maxima ("maxo"). This value is 
#                different from the value that is used to calculate the final 
#                peak intensity and is used only for the definition of isotope 
#                clusters
# filter.valid : Filter output formulas on validity of N-rule
# filter.score : Filter output formulas based on probability (see publication of
#                "SIRIOUS")
# filter.dbe   : Filter on DBE rule, NYI
# write.output : Export output? If not NULL a valid filename should be given
# more.adducts : Try to find more adducts on the given dataset by also running 
#                groupCorr alone in the raw xcms objet without groupFWHM first? 
#                If yes, it should be set to TRUE, however, the result is not 
#                guaranteed and it takes REALLY a lot of time for large datasets
# fail.rules   : What to do if the rules file fails to be read. Defaults to 
#                "internal" for constructing and internal rule for M+nH adducts 
#                only (n up to 3) or NULL for using the default from CAMERA 
#                package
# run.par      : Run in parallel what can be run in parallel
# export.what  : What other values to export apart from formulas.
#                The following options are available:
#                   "peaks"      : Export peaks with the results of CAMERA and
#                                  Rdisop
...
# find.adducts : Use the findAdducts column of CAMERA to calculate ion 
#                additions. Defaults to "TRUE", not used in the default method 
#                and used only for compatibility with annotatePeaks.old
#
...
#
# Note : The new annotation pipeline includes an iterative procedure containing 
#        the calling of each one of groupFWHM and groupCorr as well as both of 
#        them. In order not to deconvolute 3 times, since isotopes are been 
#        taken only from the groupFWHM output we change the annotation routine 
#        to include this procedure. So, if someone wishes to use only "corr" or 
#        "fwhm" for finding compounds, the routine will revert to the
#        annotatePeaks.old function in the end of the file.
#
# TODO : Implement DBE filter
#

annotatePeaks <- function(xset,group="both",iso.flow="fwhm",sigma=6,perfwhm=0.6,
    pval=0.05,cor_eic_th=0.75,find.adducts=TRUE,maxiso=5,mzabs.add=0.01,
    mzabs.fiso=0.01,mzabs.diso=0.001,ppm.fiso=10,ppm.diso=5,polarity="positive",
    rulefile=NULL,filter.valid="valid",filter.score=0.75,peak.val="maxo",
    filter.dbe=NULL,write.output=NULL,run.par=TRUE,more.adducts=FALSE,
    fail.rules="internal",export.what="all") {
    # Library checking
    if (!require(xcms))
    ...
    if (!require(Rdisop))
        stop("Bioconductor package Rdisop is required!")

    # Some input checking
    if (!("fwhm" %in% group) && !("corr" %in% group) && !("both" %in% group))
        stop("group should be one of \"fwhm\", \"corr\" or \"both\"")
    if (!is.numeric(maxiso) || !is.numeric(mzabs.add) || !is.numeric(mzabs.diso)
        || !is.numeric(mzabs.fiso) || !is.numeric(ppm.fiso) 
        || !is.numeric(ppm.diso) || !is.numeric(filter.score)
        || !is.numeric(sigma) || !is.numeric(perfwhm) 
        || !is.numeric(cor_eic_th))
        stop("maxiso, mzabs.add, mzabs.fiso, mzabs.diso, ppm.fiso, ppm.diso, ",
            "sigma, perfwhm and filter.score must be numeric")
    if (filter.score<0 || filter.score>1)
        stop("filter.score represents a probability so it must be between 0 ",
            "and 1")

    # Issue a small warning regarding the more.adducts input variable
    if(more.adducts && nrow(xset@peaks)>3000) {
        wmsg <- paste("The number of peak group is rather large and groupCorr ",
            "will be slow... Consider setting more.adducts to FALSE...")
        warning(wmsg,call.=FALSE)
    }

    if (run.par) {
        if(!require(parallel)) {
            message("R package parallel is not installed, assuming there are ",
                "at least 2 cores...")
            n.core <- 2
        }
        else {
    ...
    if (group %in% c("fwhm","corr")) {
        peaklist <- annotatePeaksOld(xset,group=group,sigma=sigma,
            perfwhm=perfwhm,pval=pval,cor_eic_th=cor_eic_th,
            find.adducts=find.adducts,maxiso=maxiso,mzabs.add=mzabs.add,
            mzabs.iso=mzabs.iso,ppm=ppm,polarity=polarity,rulefile=rulefile,
    ...
    message("Finding adducts...")
    rul <- tryCatch(read.csv(rulefile),error=function(e) { # If problems there are...
        if (!is.null(fail.rules) && fail.rules=="internal") {
            message("Problem with reading rulefile or not given... Using ",
                "internal for M+nH ONLY!")
            rul <- rules.int()
        }
        else {
            message("Problem with reading rulefile... Default from CAMERA ",
                "will be used...")
            rul <- NULL
        }
    },finally="")
    message("Finding adducts using both EIC correlation methods...")
    xsa.both <- findAdducts(xsa.both,mzabs=mzabs.add,polarity="positive",
        rules=rul)
    message("Finding adducts using FWHM grouped only...")
    xsa.fwhm <- findAdducts(xsa.fwhm,mzabs=mzabs.add,polarity="positive",
        rules=rul)
    if (more.adducts) {
        message("Finding adducts using EIC correlation only... It might take ",
            "long, please wait...")
        xsa.corr <- findAdducts(xsa.corr,mzabs=mzabs.add,polarity="positive",
            rules=rul)
    }

    if (iso.flow=="both") {
        message("Calculating new isotopic distribution...")
        xsa.both <- findIsotopes(xsa.both,maxiso=maxiso,mzabs=mzabs.fiso,
            ppm=ppm.fiso)
        peaklist <- getPeaklist(xsa.both)
        addu.fwhm <- getPeaklist(xsa.fwhm)$adduct
        addu.both <- peaklist$adduct
        message("Getting isotope clusters...")
    ...
    message("Adding monoisotopic corrected masses from isotopes...")
    ...
    for (i in monoiso.ind) {
        ...
        iso.index <- as.numeric(substring(peaklist$isotopes[i],m+1,
            attr(m,"match.length")-1))
        real.mass[i] <- as.character(round(isolist[[iso.index]]$mass,digits=6))
    }

    message("Adding monoisotopic corrected masses from calculated adducts...")
    for.fill <- setdiff(which(all.adducts!=""),grep("\\[M\\]",peaklist$isotopes))
    for (i in for.fill) {
        m <- gregexpr("(([0-9]+\\.[0-9]*)|(\\.[0-9]+))([eE][+-]?[0-9]+)?",
            peaklist$adduct[i],perl=TRUE)
        m <- m[[1]]
        tmp.mass <- numeric(length(m))
        at <- attr(m,"match.length")
        for (j in 1:length(m))
            tmp.mass[j] <- as.numeric(substring(peaklist$adduct[i],m[j],
                m[j]+at[j]-1))
    ...
    for (i in 1:nrow(peaklist)) {
        ...
        if (iso!="") {
            ...
            if (length(filt.ind[[iso.index]])>0) {
                formulae <- correctCharge(mol.list[[iso.index]]$formula[
                    filt.ind[[iso.index]]],isolist[[iso.index]]$charge,"H")
    ...
    peaklist <- exportPeaks(export.what,write.output,peaklist,real.mass,
        prop.formula,theor.mass,delta.ppm)
    
    return(peaklist)
}

exportPeaks <- function(export.what,write.output,peaklist,real.mass,
    prop.formula,theor.mass,delta.ppm) {
    ...
    if (!is.null(write.output)) {
        message("Exporting...")
        write.table(peaklist,file=write.output,quote=FALSE,sep="\t",
            row.names=FALSE)
    }
    return(peaklist)
}
...
filterMolecules <- function(mol,n.rule="valid",f.score=NULL) {
    ...
    if (!is.null(n.rule)) {
        n.rule <- paste(toupper(substring(n.rule,1,1)),substring(n.rule,2),
            sep="")
    ...
    switch(type,
        ESI = {
            ...
            for (i in 1:length(iso))
                iso[[i]]$mass <- 
                as.numeric(iso[[i]]$charge*(iso[[i]]$peaks[1,1]-the.proton.mass))
        },
        CI = { iso <- iso })
    return(iso)
}

to.ppm <- function(obs,ex) { 
    return(abs((obs-ex)/ex*1e+6)) 
}
...
my.xsAnnotate <- function (xs=NULL,sample=NA,nSlaves=1,paral="snow") {
    ...
    if (length(sampnames(xs)) > 1) {
        if (!nrow(xs@groups) > 0)
            stop("First argument must be a xcmsSet with group information or ",
        ...
        if (is.null(sample) || is.na(sample))
            ...
            if (sample == -1)
                ...
                stop("Parameter sample must be lower equal than number of ",
                    "samples and greater than 0.\n")
            else
                object@sample <- sample
        }
        ...
        if (paral=="Rmpi") {
            if (require(rmpi,character.only = TRUE) && !is.null(nSlaves)) { # Try Rmpi first
                if (is.loaded("mpi_initialize")) {
                    if (mpi.comm.size() > 0) {
                        warning("There are already intialized mpi slaves on ",
                        ...
                        if (mpi.comm.size() > 1)
                        {
                            ...
                            warning("Spawning of mpi slaves have failed. ",
                                "CAMERA will run without parallelization.\n")
                    }
                }
                else {
                    snow = "snow"
                    if (try(require(snow,character.only=TRUE,quietly=TRUE))) {
                        cat("Starting snow cluster with",nSlaves,
                            "local sockets.\n")
...
my.getPeaks_selection <- function(xs,method="medret",value="into") {
    if (nrow(xs@groups) > 0) {
        groupmat <- groups(xs)
        ts <- data.frame(cbind(groupmat,groupval(xs,method=method,value=value)),
            row.names=NULL)
        cnames <- colnames(ts)
        if (cnames[1] == 'mzmed') 
            cnames[1] <- 'mz' 
        ...
        ts <- xs@peaks
    else stop ('First argument must be a xcmsSet with group information or ",
        "contain only one sample.')
    return(as.matrix(ts))
}
...
annotatePeaksOld <- function(xset,group="fwhm",sigma=6, perfwhm=0.6,
    cor_eic_th=0.75,pval=0.05,find.adducts=FALSE,maxiso=5,mzabs.add=0.01,
    mzabs.iso=0.001,ppm=5,polarity="positive",rulefile=NULL,
    filter.valid="valid",filter.score=0.75,peak.val="maxo",filter.dbe=NULL,
    export.what="all") {
    # CAMERA stuff
    message("Grouping peaks to define isotope clusters...")
    xsa <- xsAnnotate(xset)
    switch(group,
        fwhm = { 
            xsa <- groupFWHM(xsa,sigma=sigma,perfwhm=perfwhm)
    ...
    message("Reconstructing peak list...")
    ...
    for (i in 1:dim(peaklist)[1]) {
        ...
        if (iso!="") {
            ...
            iso.index <- as.numeric(substring(iso,m+1,attr(m,"match.length")-1))
            if (length(filt.ind[[iso.index]])>0) {
                formulae <- correctCharge(
                    mol.list[[iso.index]]$formula[filt.ind[[iso.index]]],
                ...
                theor.mass[i] <- paste(masses,collapse=" ,")
                if (length(grep("\\[M\\]",iso))>0) { # Monoisotopic peak
                    delta.ppms <- to.ppm(peaklist[i,"mz"],
                        getMassAlt(mol.list[[
                            iso.index]]$formula[filt.ind[[iso.index]]]))
                    delta.ppms <- round(delta.ppms,digits=6)
                    delta.ppm[i] <- paste(delta.ppms,collapse=", ")
                }
            }
    ...
    peaklist <- exportPeaks(export.what,write.output,peaklist,prop.formula,
        theor.mass,real.mass,delta.ppm)
...
trackValues <- function(i,mz,alt.mass,real.mass,delta.ppms,mol.val,prop.formula,
    theor.mass) {
    cat("i :",i,"\n")
    cat("mz :",mz,"\n")
    cat("th :",alt.mass,"\n")
