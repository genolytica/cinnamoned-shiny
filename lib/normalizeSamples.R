...
# time.range : a list with time ranges for each sample (remove salt and plastic 
#              after visual inspection)

# peaks      : object returned from xcmsPipeline, single or list
# time.range : a list with time ranges for each sample (remove salt and plastic 
#              after visual inspection)
# cutrat     : a threshold to exclude possible non-stable normalizers (in log2 
#              that means ratio)
# cutq       : a quantile to exclude non-aligned RT points
# correctfor : both for both, time for only RT correction, intensity for only 
#              intensity correction and none for just m/z matching

normalizeSamples <- function(peaks,dbdata,method=c("geom","rlm","both"),
    ...
    if (!is.element(norm,c("loess","rlm","lm","simple","cluster")))
        stop("method must be one of \"loess\", \"rlm\", \"lm\", \"simple\" or ",
            "\"cluster\"!")
    correctfor <- tolower(correctfor[1])
    if (!is.element(correctfor,c("both","time","intensity","none")))
        stop("correctfor must be one of \"both\", \"time\", \"intensity\",or ",
            "\"none\"!")
    ...
    if (is.character(tspan)) {
        tspan <- tolower(tspan)
        if (tspan != "adaptive")
            stop("Span for RT correction must be \"adaptive\" when not ",
                "numeric!")
    }
    else if (is.numeric(tspan)) {
        if (tspan<0 || tspan>1)
            stop("RT correction span must be a number between 0 and 1, when ",
                "numeric!")
    ...
    if (is.character(ispan)) {
        ispan <- tolower(ispan)
        if (ispan != "adaptive")
            stop("Intensity correction span must be \"adaptive\" when not ",
                "numeric!")
    }
    else if (is.numeric(ispan)) {
        if (ispan<0 || ispan>1)
            stop("Intensity correction span must be a number between 0 and 1, ",
                "when numeric!")
        if (ispan==0) ispan <- "adaptive"
    }
    if (cutq<0 || cutq>1)
        stop("The RT deviation exlusion quantile must be a number between 0 ",
            "and 1, when numeric!")
    ...
    if (!is.null(time.range)) {
        if (length(time.range)!=length(peaks))
            stop("The filtering time intervals list should have the same ",
                "length as the list of peak tables!")
        for (i in 1:length(peaks)) {
            rem.idx <- which(peaks[[i]]$rt<time.range[[i]][1] 
                | peaks[[i]]$rt>time.range[[i]][2])
            if (length(rem.idx)>0)
                peaks[[i]] <- peaks[[i]][-rem.idx,]
        }
    }

    # Get normalizers and reference data from the database and create the 
    # match lists
    ...
    for (i in 1:length(peaks)) {    
        ...
        match.ref[[i]]$pct.iset <- 
            sprintf("%.3f",100*length(iset.match[[i]])/length(iset.ref))
        ...
        if (correctfor=="both" || correctfor=="time") {
            ...
            rtclist <- is.retcor(rtnew,rtref,iset,tspan=tspan,it=tit,
                corrfac=corrfac)
            rtcor <- rtclist$rtcor
        ...
        matched[[i]]$rawinten[match.ref[[i]]$ref.idx] <- 
            peaks[[i]]$into[match.ref[[i]]$new.idx]
        # Intensities
        ...
        if (correctfor=="both" || correctfor=="intensity")
        {
            ...
            intcor <- is.intcor(intnew,intref,iset,method=normalize,span=ispan,
                corrfac=corrfac,adapt.factor=length(iset.ref))
            #intcor <- intc$intcor
            peaks[[i]]$into[!is.na(peaks[[i]]$into)] <- 2^intcor
            match.ref[[i]]$pct.iset.inten <- sprintf("%.3f",
        ...
        matched[[i]]$mz[match.ref[[i]]$ref.idx] <- 
            peaks[[i]]$mz[match.ref[[i]]$new.idx]
        matched[[i]]$rt[match.ref[[i]]$ref.idx] <- 
            peaks[[i]]$rt[match.ref[[i]]$new.idx]
        matched[[i]]$norminten[match.ref[[i]]$ref.idx] <- 
            peaks[[i]]$into[match.ref[[i]]$new.idx]
    ...
    pcts <- do.call("rbind",lapply(match.ref,function(x) {
        return(as.numeric(c(x$pct,x$pct.iset,x$pct.iset.rt,x$pct.iset.inten)))
    }))
    ...
    if (!is.null(diagplot)) {
        ...
        n <- norm$norminten
        n[n==0] <- NA
        boxplot.mat(u,name=name,output=plottype,
            fil=file.path(diagplot,paste("boxplot_raw.",plottype,sep="")),
            main="log2 intensity distributions - raw",cex.main=0.9)
        boxplot.mat(n,name=name,output=plottype,fil=file.path(diagplot,
            paste("boxplot_normalized.",plottype,sep="")),
    ...
    if (!is.null(export) && export.type!="none") {
        ...
        if (export.type=="all") {
            ...
            nam[1] <- "id"
            names(final) <- nam
            write.table(final,file=export,quote=FALSE,sep="\t",na="NaN",
                row.names=FALSE)
        }
        #write.table(final,file=export,quote=FALSE,sep="\t",na="-",
        #row.names=FALSE)
