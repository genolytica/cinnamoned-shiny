# An R file containing several functions for querying some metabolomic 
# databases. Currently, HMDB, ChEBI and KEGG Compound. This might evolve in the 
# future to be able to handle local databases (currently trying for METLIN)
#
...
# Query KEGG compound database, given either one mass and a tolerance (in Da) to 
# create a search interval, or two masses that will form an interval
# TODO: Somehow incorporate RT and MZ values in ID???
#
# IMPORTANT! This function is now outdated (2018) as KEGGSOAP has been removed
# long ago from Bioconductor. The package to work with now is KEGGREST.
queryKEGG.mass.old <- function(m,m.tol=0.01,m.int=NULL) {
    if (!require(KEGGSOAP))
    ...
    if (!missing(m) && !is.null(m.int)) {
        warning("Only ONE of m or m.int can be specified! Proceeding with ",
            "m.int")
        m <- NULL
    }
    ...
    message(paste("Querying KEGG COMPOUND for m between",m-m.tol,"and",m+m.tol,
        "...",sep=" "))
    comps <- search.compounds.by.mass(m,m.tol)
    if (length(comps)==0) {
        cat("No results found for m between",m-m.tol,"and",m+m.tol,
            "in KEGG COMPOUND.\n",sep=" ")
        return("")
    }
...
# Query ChEBI database, given either one mass and a tolerance (in Da) to create
# a search interval, or two masses that will form an interval
...
queryChEBI.mass <- function(m,m.tol=0.01,m.int=NULL,dbdata=NULL) {
    ...
    message(paste("Querying ChEBI for m between",m-m.tol,"and",m+m.tol,"...",
        sep=" "))
    
    con <- dbConnect(MySQL(),user=dbdata[1],password=dbdata[2],dbname="ChEBI",
        host="localhost")
    
    mass.query <- paste("SELECT chemical_data, compound_id FROM chemical_data ",
        "WHERE type='MASS' AND (chemical_data>",m-m.tol," AND chemical_data<",
        m+m.tol,")",sep="")
    mass.comp <- dbGetQuery(con,mass.query)

    if (nrow(mass.comp)==0) {
        cat("No results found for mass between",m-m.tol,"and",m+m.tol,
            "in ChEBI.\n",sep=" ")
        dbDisconnect(con)
        return("")
    }
    else {
        # Charge correction is not required... Average mass is recorded, 
        # independently of charge
        # Query for the formulas
        form <- character(nrow(mass.comp))
        compound_ids <- paste(mass.comp$compound_id,collapse=",")
        formula.query <- paste("SELECT chemical_data.compound_id,",
            "chemical_data,name FROM chemical_data ",
...
# Query a local copy of the HMDB database, created using the raw metabocards 
# file provided by the HMDB website, given either one mass and a tolerance 
# (in Da) to create a search interval, or two masses that will form an interval
# dbdata : username and password for the database
# mono   : search weight_mono or weight_average
# bioflu : Add a biofluid filter e.g. Urine
# exact  : If TRUE when used with bioflu then only hits appearing only in bioflu 
#          and not other bioflus are returned
queryHMDB_Compact.mass <- function(m,m.tol=0.01,m.int=NULL,dbdata=NULL,
    mono=TRUE,bioflu=NULL,exact=FALSE) {
    if (!require(RMySQL))
        stop("R package RMySQL is required!")

    if (missing(m) && is.null(m.int))
        stop("Either a specific mass value or a mass interval MUST be ",
            "specified!")
    if (!missing(m) && !is.null(m.int)) {
        warning("Only ONE of m or m.int can be specified! Proceeding with ",
            "m.int")
    ...
    message(paste("Querying HMDB for m between",m-m.tol,"and",m+m.tol,"...",
        sep=" "))
    con <- dbConnect(MySQL(),user=dbdata[1],password=dbdata[2],
        dbname="HMDB_Compact",host="localhost")
    ...
    if (!is.null(bioflu)) {
        ...
        form.query <- paste("SELECT hmdb_id,chemical_formula,name ",
            "FROM HMDB_Essential WHERE weight_",w.s,">",m-m.tol," AND weight_",
            w.s,"<",m+m.tol," AND biofluid_location ",bioflu.like," '",
            bioflu,"'",sep="")
    }
    else
        form.query <- paste("SELECT hmdb_id,chemical_formula,name ",
...
queryHMDB.mass <- function(m,m.tol=0.01,m.int=NULL) {
    ...
    message(paste("Querying HMDB for mass between",m[1],"and",m[2],"...",
        sep=" "))
    hmdb.page <- postForm(url,"query_from"=as.character(m[1]),
        "query_to"=as.character(m[2]),"search"="molecular_weight")
    s.p <- scrape(object="hmdb.page")
    
    hmdb <- tryCatch(paste(sub("\n\t\n",xpathSApply(s.p[[1]],
        "//table//tr/td[@rowspan=\"2\"]",xmlValue)),collapse=", "),
        error=function(e) {
            cat("No results found for mass between",m[1],"and",m[2],
                "in HMDB online.\n",sep=" ")
        },finally="")
    return(hmdb)
}

# Query the METLIN database, given either one mass and a tolerance (in Da) to 
# create a search interval, or two masses that will form an interval
...
queryMETLIN.mass <- function(m,m.tol=0.01,m.int=NULL) {
    ...
    message(paste("Querying METLIN for m between",m-m.tol,"and",m+m.tol,"...",
        sep=" "))
    metlin.page <- postForm(url,"mass_mid"=as.character(m),
        "mass_tol"=as.character(m.tol),"mass_mode"="0")
    assign("metlin.page",metlin.page)
    scr.page <- scrape(object="metlin.page")

    metlin <- tryCatch(paste(xpathSApply(scr.page[[1]],
        "//table/tr/td[1]/b/a",xmlValue),collapse=", "),error=function(e) {
        cat("No results found for mass between",m-m.tol,"and",m+m.tol,
            "in METLIN online.\n",sep=" ")
    },finally="")
    return(metlin)
...
strip.kegg.formula.name <- function(comp) {
    ...
    if (ind.form-ind.name>1) { # Multiple names
        ...
        names.clear <- gsub("^\\s+|\\s+$|\\s+","",substr(names.dirty,5,
            nchar(names.dirty)))
    }
    else # One name
        names.clear <- gsub("^\\s+|\\s+$|\\s+","",substr(entry[[1]][ind.name],5,
            nchar(entry[[1]][ind.name])))
...
getdbdata <- function() {
    out <- tryCatch({ d <- read.table("db.dat",colClasses="character")
              as.character(c(d[1,],d[2,])) },error=function(e) { 
                  c("bWV0YWJvX25vYm9keQ==","eWRvYm9uX29iYXRlbQ==") 
    },finally="")
    return(out)
}
...
#{
#   charge.query = paste("SELECT chemical_data FROM chemical_data ",
#       "WHERE type='CHARGE' AND compound_id=",mass.comp$compound_id[i],sep="")
#   charge[i] <- as.numeric(dbGetQuery(con,charge.query))
#   if (charge[i]>0)
#       mass.comp$chemical_data[i] <- 
#           charge[i]*(as.numeric(mass.comp$chemical_data[i]) - the.proton.mass)
#   else if (charge[i]<0)
#       mass.comp$chemical_data[i] <- 
#           abs(charge[i])*(as.numeric(mass.comp$chemical_data[i]) + 
#               the.proton.mass)
#}
#mass.comp <- cbind(mass.comp,charge)
## Recheck the masses now
#ind <- which(as.numeric(mass.comp$chemical_data)<m-m.tol 
#   || as.numeric(mass.comp$chemical_data)>m+m.tol)
#if (length(ind)!=0)
