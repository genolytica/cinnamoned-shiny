################################################################################
# A note... It is not really possible to filter intensity at the very raw data #
# level... The data would become too sparse... What may be done is to convert  #
# intensities above or below a maximum/minimum value to a constant value, the  #
# one given. We should forget generally the missing values... They are not     #
# well tolerated by analysis software...                                       #
################################################################################

# filterBrukerNetCDF is an R function that accepts as input a set of M/S NetCDF 
# files and additional raw data filtering parameters and rewrites the files 
# having filtered them based on the requested filters.
#
...
# filter.time  : The minimum and maximum retention time (in seconds) below and 
#                above of which the M/S data should be truncated. If filtering 
#                not wished, it should be NULL, else a vector of length 2. 
#                Defaults to c(900,3000).
# filter.mz    : The minimum and maximum m/z values below and avove of which the 
#                M/S data will be truncated for each M/S scan. If filtering not 
#                wished, it should be NULL, else a vector of length 2. Defaults 
#                to NULL (no filtering).
# filter.inten : The baseline and saturated intensity that M/S below and above 
#                these values data will be outputed for each M/S scan. If this 
#                procedure is not wished, it should be NULL, else a vector of 
#                length 2. Defaults to NULL.
# data.time    : In what unit the retention time lives in the data. Defaults to 
#                seconds.
# 
# 
...
filterBrukerNetCDF <- function(cdf.files,out.path=NULL,filter.time=c(900,3000),
    filter.mz=NULL,filter.inten=NULL,missing.value=-9999,data.time="seconds",
    ...
    for (cdf in cdf.files) {
        message("Now processing file ",cdf,"...")
        ########################################################################
        updateShinyProgressBar(
            shinyProgressData=shinyProgressData,
            pbValue=2,
            headerMsg=paste("Filtering input file ",basename(cdf),"...",
                collapse=""),
            footerMsg=""
        )
        ########################################################################
        ...
        # General variables
        message("Getting general variables...")
        ########################################################################
        updateShinyProgressBar(
            shinyProgressData=shinyProgressData,
            pbValue=2,
            headerMsg=paste("Filtering input file ",basename(cdf),"...",
                collapse=""),
            footerMsg="Getting general variables..."
        )
        ########################################################################
        ...
        # Measurements
        message("Getting actual measurements...")
        ########################################################################
        updateShinyProgressBar(
            shinyProgressData=shinyProgressData,
            pbValue=3,
            headerMsg=paste("Filtering input file ",basename(cdf),"...",
                collapse=""),
            footerMsg="Getting actual measurements..."
        )
        ########################################################################
        ...
        # Get dummy variables
        message("Getting dummy variables...")
        ########################################################################
        updateShinyProgressBar(
            shinyProgressData=shinyProgressData,
            pbValue=4,
            headerMsg=paste("Filtering input file ",basename(cdf),"...",
                collapse=""),
            footerMsg="Getting dummy variables..."
        )
        ########################################################################
        ...
        tryCatch({
            ...
            message(":units global attribute is not defined... User input or ",
                "default will be used")
            time.unit <- default.time.unit
        },finally="")
        # We must deal with the case time being in minutes
        if (length(grep("min",time.unit,ignore.case=TRUE))!=0 & 
            !is.null(filter.time))
            filter.time <- 60*filter.time
        
        # Filter the time
        if (!is.null(filter.time)) {
            message("Filtering retention time...")
            ####################################################################
            updateShinyProgressBar(
                shinyProgressData=shinyProgressData,
                pbValue=5,
                headerMsg=paste("Filtering input file ",basename(cdf),"...",
                    collapse=""),
                footerMsg="Filtering retention time..."
            )
            ####################################################################
        ...
        # -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        # We must also exclude the m/z and intensity corresponding to excluded 
        # RT (retention time)
        # Whether time.ind is sorted or not we must update scan.index in every 
        # step
        # 1. Create 2 list objects with mass/intensity containing a scan in each 
        #    plot
        # 2. Delete the list members given by the retention time excluding 
        #    indices
        ...
        # 1.
        message("Getting m/z and intensity in list objects...") 
        ########################################################################
        updateShinyProgressBar(
            shinyProgressData=shinyProgressData,
            pbValue=6,
            headerMsg=paste("Filtering input file ",basename(cdf),"...",
                collapse=""),
            footerMsg="Getting m/z and intensity in list objects..."
        )
        ########################################################################
        ...
        if (!is.null(filter.mz)) {
            # Filter
            message("Filtering m/z values...")
            for (i in 1:n.points) {
                filter.mass <- which(l.mass[[i]]<filter.mz[1] 
                    | l.mass[[i]]>filter.mz[2])
        ...
        if (!is.null(filter.inten)) {
            # Filter
            message("Filtering intensity values...")    
            for (i in 1:n.points) {
                ...
                # If intensity has been filtered, we must recalculate 
                # total_intensity
        ...
        # 5.
        message("Recalculating scan indices and scan point counts...")
        ########################################################################
        updateShinyProgressBar(
            shinyProgressData=shinyProgressData,
            pbValue=7,
            headerMsg=paste("Filtering input file ",basename(cdf),"...",
                collapse=""),
            footerMsg="Recalculating scan indices and scan point counts..."
        )
        ########################################################################
        ...
        # 5.
        message("Collapsing m/z and intensity lists...")
        ########################################################################
        updateShinyProgressBar(
            shinyProgressData=shinyProgressData,
            pbValue=8,
            headerMsg=paste("Filtering input file ",basename(cdf),"...",
                collapse=""),
            footerMsg="Collapsing m/z and intensity lists..."
        )
        ########################################################################
        ...
        message("Assigning possible missing data values...")
        ########################################################################
        updateShinyProgressBar(
            shinyProgressData=shinyProgressData,
            pbValue=9,
            headerMsg=paste("Filtering input file ",basename(cdf),"...",
                collapse=""),
            footerMsg="Assigning possible missing data values..."
        )
        ########################################################################
        ...
        updateShinyProgressBar(
            ...
            headerMsg=paste("Filtering input file ",basename(cdf),"...",
                collapse=""),
            footerMsg="Writing global attributes..."
        )
        ########################################################################
        
        global.attributes <- c("dataset_completeness","ms_template_revision",
            "netcdf_revision","languages","netcdf_file_date_time_stamp",
            "experiment_title","experiment_date_time_stamp","operator_name",
            "pre_experiment_program_name","post_experiment_program_name",
            "source_file_reference","source_file_format","experiment_type",
            "sample_state","test_separation_type","test_ms_inlet",
            "test_ionization_mode","test_ionization_polarity",
            "test_detector_type","test_resolution_type","test_scan_function",
            "test_scan_direction","test_scan_law","raw_data_mass_format",
            "raw_data_time_format","raw_data_intensity_format","units",
            "scale_factor")
        for (i in global.attributes)                       
            att.copy.nc(o,"NC_GLOBAL",i,out,"NC_GLOBAL")
        
        # Redefine static dimensions
        message("Writing dimensions...")
        ########################################################################
        updateShinyProgressBar(
            shinyProgressData=shinyProgressData,
            pbValue=11,
            headerMsg=paste("Filtering input file ",basename(cdf),"...",
                collapse=""),
            footerMsg="Writing dimensions..."
        )
        ########################################################################
        
        dimensions <- c("_2_byte_string","_4_byte_string","_8_byte_string",
            "_16_byte_string","_32_byte_string","_64_byte_string",
            "_128_byte_string","_255_byte_string","range","error_number",
            "instrument_number")
        for (i in dimensions)
        ...
        # Redefine all the variables (and attributes), can't loop because of 
        # different dimensions... :-@
        message("Defining variables and their attributes...")
        ########################################################################
        updateShinyProgressBar(
            shinyProgressData=shinyProgressData,
            pbValue=12,
            headerMsg=paste("Filtering input file ",basename(cdf),"...",
                collapse=""),
            footerMsg="Defining variables and their attributes..."
        )
        ########################################################################
        
        var.def.nc(out,"error_log","NC_CHAR",c("_64_byte_string",
            "error_number"))
        ...
        att.put.nc(out,"total_intensity","missing_value",
            missing.type,missing.value)
        att.put.nc(out,"total_intensity","units","NC_CHAR",
        ...
        att.put.nc(out,"intensity_values","missing_value",
            missing.type,missing.value)
        att.put.nc(out,"intensity_values","units","NC_CHAR",
            "Arbitrary Intensity Units")
        att.put.nc(out,"intensity_values","scale_factor","NC_DOUBLE",1)
        att.put.nc(out,"intensity_values","scale_factor","NC_DOUBLE",1)
        # Some here that can be looped
        vars.instrument <- c("instrument_name","instrument_id","instrument_mfr",
            "instrument_model","instrument_serial_no","instrument_sw_version",
            "instrument_fw_version","instrument_os_version",
            "instrument_app_version","instrument_comments")
        for (i in vars.instrument)
            var.def.nc(out,i,"NC_CHAR",c("_32_byte_string","instrument_number"))    

        # Fill variables
        message("Writing variable values...")
        ########################################################################
        updateShinyProgressBar(
            shinyProgressData=shinyProgressData,
            pbValue=13,
            headerMsg=paste("Filtering input file ",basename(cdf),"...",
                collapse=""),
            footerMsg="Writing variable values..."
        )
        ########################################################################
        ...
        var.put.nc(out,"instrument_serial_no",
            var.get.nc(o,"instrument_serial_no"))
        var.put.nc(out,"instrument_sw_version",
            var.get.nc(o,"instrument_sw_version"))
        var.put.nc(out,"instrument_fw_version",
            var.get.nc(o,"instrument_sw_version"))
        var.put.nc(out,"instrument_os_version",
            var.get.nc(o,"instrument_os_version"))
        var.put.nc(out,"instrument_app_version",
            var.get.nc(o,"instrument_app_version"))
        var.put.nc(out,"instrument_comments",
            var.get.nc(o,"instrument_comments"))
        ...
        updateShinyProgressBar(
            ...
            headerMsg=paste("Filtering input file ",basename(cdf),"...",
                collapse=""),
            footerMsg="Clearing garbage..."
        )
        ########################################################################
        
        clear()
        message(paste("Output written to",createOutFile(cdf,out.path)))
        message("Finished!") 
    }
}
