# An R file containing a framework for peak intensity normalization for the 
# specific needs of the INSERM project. Briefly, it uses normalization 
# algorithms similar (or same) to the concept of rank invariant normalization 
# taken from the microarray bibliography. It should return, apart from the 
# normalized values, also the indices of the rank invariant entities, as these 
# constitute the basis of the data mining problem that will define the final
...
# method       : Which rank invariant method to use? Can be one of "raffy" for 
#                the method from affy package, "rlumi" for the method from lumi 
#                package or "rgrsn" for the method presented in Pelz et al BMC 
#                paper (2008). From our methods, can be also "rrlm" or "rgeom"
...
normalizePeaks <- function(peaks,method="raffy",...) {
    if (!is.element(method,c("raffy","rlumi","rgrsn","rrlm","rgeom")))
        stop("Normalization method must be one of \"raffy\", \"rlumi\", ',
            '\"rgrsn\", \"rrlm\" or \"rgeom\"!")
...
bootstrap.IS <- function(peaks,baseline=NULL,iset,method="raffy",opt="data",
    measure="mad",n=100,multicore=FALSE,...) {
    require(utils)
    ...
    if (multicore && !require(parallel)) {
        message("Package parallel required to run in parallel... Switching to ",
            "single core...")
        multicore <- FALSE
    }
...
# type : should we normalize as affy normalized by default (that is each array 
#        against the baseline) (pairwise) or gather all the invariant set and 
#        use it for normalization (total), the latter is more fair for other 
#        methods and also what we want to do
ri.norm.affy <- function(peaks,baseline=NULL,pct=60,type=c("pairwise","total"),
    out.scale="log2",mcalc=c("none","mad","qn","sn"),...) {
    if (!require(affy))
        stop("Bioconductor package affy is required!")
    ...
    if (type=="pairwise") {
        for (i in 1:ncol(peaks)) {
            #pred <- approx(tmp[[i]]$n.curve$y,tmp[[i]]$n.curve$x,
            #   xout=peaks[,i],rule=2)
            #n.peaks[,i] <- as.numeric(pred$y)
            # Use real extrapolation
            pred <- extrap1(tmp[[i]]$n.curve$y,tmp[[i]]$n.curve$x,xo=peaks[,i]) 
            n.peaks[,i] <- pred
        }
    }
    else if (type=="total") {
    ...
    if (mcalc=="none")
        attr(n.peaks,"ri.info") <- list(ri.set=ri.set.fi,ri.all=ri.set,
            metric=NULL,imetric=NULL,rcv=NULL,ircv=NULL,w.metric=NULL,
            w.imetric=NULL,w.rcv=NULL,w.ircv=NULL)
    else {
        ...
        attr(n.peaks,"ri.info") <- list(ri.set=ri.set.fi,ri.all=ri.set,
            metric=NULL,imetric=NULL,rcv=NULL,ircv=NULL,w.metric=w.metric,
            w.imetric=w.imetric,w.rcv=w.rcv,w.ircv=w.ircv)
    }
...
# Hack a bit the rank invariant function of lumi in order to return the index or 
# RIs. Also, we change the defaults for the more messageersed metabolomic peak 
# data
ri.norm.lumi <- function (x.lumi,targetArray=NULL,rrc=0.05,
    lowRank=seq(0.1,0.05,-0.01),highRank=0.5,minSize=0.02,maxit=200,
    out.scale="log2",mcalc=c("none","mad","qn","sn")) {
    if (is(x.lumi,"ExpressionSet"))
        ...
        stop("The object should be a matrix or class \"ExpressionSet\" ",
            "inherited!")
    ...
    if (!is.null(targetArray)) {
        ...
        if (length(targetArray)>1) {
            if (length(targetArray)!=nrow(expr)) 
                stop("targetArray should be an index or a vector has the same ",
                    "length as other samples.")
        ...
        if (is.numeric(targetArray)) {
            if (targetArray<1 || targetArray>nrow(expr)) {
                warning("The provided targetArray is invalid and will be set ",
                    "as NULL!")
                targetArray <- NULL
            }
        }
    ...
    ri <- rep(F,nrow(expr))
    rankscores <- apply(er,2,function(r,v) {
        return(abs((r-v)/length(v)))
    },er[,targetArray])
    ...
    for (i in 1:nArray) {
        ...
        for (lowrank in lowRank) {
            if (getOption("verbose")) 
                cat("Sample",i-ifelse(externalTarget, 1, 0),"using lowrank ",
                    lowrank,"\n")

            ri <- rankscores[,i]<rrc & er[,i]>nrows*lowrank & 
                er[,i]<nrows*highRank
            if (getOption("verbose"))
                cat("Nr rankinvariant probes=",sum(ri),"\n")
            if (sum(ri)>=minSize*nrows) {
                coef <- rlm(expr[ri,targetArray] ~ expr[ri,i],psi=psi.bisquare,
                    maxit=maxit)$coef
        ...
        if (lowrank==tail(lowRank,1)) 
            stop(paste("No rankinvariant set for sample", i - 
                ifelse(externalTarget,1,0), "use broader lowRank/highRank or ",
                "use another normalization method"))
    }
    if (externalTarget) 
    ...
    if (!log2Trans)
        ...
        x <- expr
    if (mcalc=="none")
        attr(x.lumi,"ri.info") <- list(ri.set=is,ri.all=NULL,metric=NULL,
            imetric=NULL,rcv=NULL,ircv=NULL,w.metric=NULL,w.imetric=NULL,
            w.rcv=NULL,w.ircv=NULL)
    else {
        ...
        attr(x.lumi,"ri.info") <- list(ri.set=which(ri),ri.all=NULL,metric=NULL,
            imetric=NULL,rcv=NULL,ircv=NULL,w.metric=w.metric,
            w.imetric=w.imetric,w.rcv=w.rcv,w.ircv=w.ircv)
    }
...
# Adapted from the code presented with the paper of 
# Pelz et al., BMC Bioinformatics, 2008
ri.norm.grsn <- function(peaks,count=ceiling(0.1*nrow(peaks)),f=0.25,
    out.scale="log2",cut.type=c("fixed","mad","qn","sn"),
    opt=c("data","standards","both"),multicore=FALSE,
    mcalc=c("none","mad","qn","sn"),minstat=c("sumq","rcv")) {
    # Check cut.type and opt
    cut.type <- tolower(cut.type[1])
    if (!is.element(cut.type,c("quantile","fixed","mad","qn","sn")))
        stop("cut.type parameter must be a \"quantile\", \"fixed\", \"mad\", ",
            "\"qn\", \"sn\"!")
    opt <- tolower(opt[1])
    ...
    # Apply our extra rule to further optimize the invariant set (starting from 
    # a bit larger set)
    if (cut.type=="fixed")
        invariantIdx <- idx
        ...
        attr(M1,"caller") <- "grsn"
        message("Pruning iteratively the number of invariant elements and ",
            "calculating evaluation metrics...")
        message("It might take some time... No C/C++ backend... ",
            "Please be patient...")
        iset.init <- idx
        iter.out <- iterative.select(M1,Mavg,iset.init,scal=cut.type,
            output="list",multicore=multicore,opt=opt,span=f,minstat=minstat)
...
# n          : number of features to acquire from each model (either pairwise or 
#              baseline) if method="fixed"
# p          : percentile of the total residuals distribution to use for  
#              dynamically selecting features if method="quantile"
# pct        : percentage of dataset variables
# cut.type   : how to select the invariant set? "quantile", "fixed" or "auto" 
#              for iterative selection
# off.set    : the offset in case of replace.zero="offset"
# dyn.select : run the position.order function in order to have a certain 
#              ordering according to feature appearance, instead of simply 
#              tabling the unlist? the latter will definitely fail if 
#              n=ncol(peaks)
# opt        : optimize the final selection based on the variability of the 
#              normalized matrix, the standards or both
# ... further inputs to rlm
rlm.norm <- function(peaks,baseline=NULL,pairwise=FALSE,
    replace.zero=c("constant","offset","minpos","minpos.noise","none"),
    cut.type=c("quantile","fixed","mad","qn","sn"),
    n=ceiling(nrow(peaks)/3),p=50,pct=60,span=0.25,multicore=FALSE,
    out.scale="log2",off.set=1,dyn.select=FALSE,
    opt=c("data","standards","both"),coverage=c("none","range"),minmax=c(100,n),
    minstat=c("sumq","rcv"),...) {
    if (!require(MASS))
        stop("R package MASS is required!")
    ...
    if (is.numeric(n)) {
        if (n>nrow(peaks)) {
            message("The size of invariant set cannot exceed the number of ",
                "variables! Using the default...")
    ...
    if (!is.element(replace.zero,c("constant","offset","minpos","minpos.noise",
        "none")))
        stop("replace.zero must be one of \"constant\",\"offset\",\"minpos\",',
            '\"minpos.noise\" or \"none\"")
    if (minstat!="sumq" && minstat!="rcv")
        stop("minstat must be one of \"sumq\" or \"rcv\"!")
    ...
    if (pairwise) { # Pairwise models in X...
        message("Calculating RLM pairwise model parameters...")
        ind <- 0
        # The actual fit
        for (i in 1:(m-1)) {
            for (j in i:m) {
                if (i!=j) {
                    ...
                    resids[[ind]] <- sort(abs(residuals(fits[[ind]])),
                        index.return=TRUE)
                }
            }
        }
        ...
        message("Calculating RLM baseline model parameters...")
        for (i in 1:m) {
    ...
    if (cut.type=="quantile") { # This should give cands of different lengths
        ...
        if (dyn.select) {
            ...
            cand.tab <- table(unlist(cands))
            tmp.set <- cand.tab[which(
                cand.tab>=ceiling((pct/100)*length(resids)))]
            tmp.set <- sort(tmp.set,decreasing=TRUE)
            i.set <- as.numeric(names(tmp.set))
        }
    }
    else if (is.element(cut.type,c("mad","qn","sn"))) {
        # In this case n is used to initialize the invariant set... But if it is 
        # significantly lower than at least 2/3rds of the dataset AND the 
        # dataset is small, the method might not converge... so issue a warning in case...
        if (n<ceiling(nrow(peaks)/3) && nrow(peaks)<1000)
            warning("n is smaller than 2/3rds of the dataset! Method might ",
                "not converge...",call.=FALSE)
        # A little tip to reduce arguments in iterative.select
        attr(X,"caller") <- "rlm"
        ...
        if (dyn.select) {
            ...
            if (n==ncol(peaks)) {
                cat("The selected cut.type will fail with n=ncol(peaks)... ",
                    "Reverting to dynamical selection")
                PM <- position.matrix(cands,nrow(X))
                iset.init <- position.order(PM)
            }
            else
        ...
        iter.out <- iterative.select(X,bl.sample,iset.init,scal=cut.type,
            output="list",multicore=multicore,span=span,opt=opt,
            coverage=coverage,minmax=minmax,minstat=minstat)
        i.set <- iter.out[[1]]
    }
    ...
    if (is.element(cut.type,c("mad","qn","sn")))
        ...
        attr(Xn,"ri.info") <- list(ri.set=i.set,ri.all=NULL,metric=NULL,
            imetric=NULL,rcv=NULL,ircv=NULL,w.metric=NULL,w.imetric=NULL,
            w.rcv=rcv.mad(tmp),w.ircv=NULL)
    }
...
#                normalized matrix, the standards or both
# pct          : percentage of dataset variables
geom.norm <- function(peaks,baseline=NULL,grid.size=5*nrow(peaks),squared=FALSE,
    p=50,n=ceiling(nrow(peaks)/3),replace.zero=c("constant","offset","minpos",
    "minpos.noise","none"),span=0.25,multicore=FALSE,
    cut.type=c("quantile","fixed","mad","qn","sn"),out.scale="log2",
    minmax=c(100,n),off.set=1,opt=c("data","standards","both"),
    coverage=c("none","range"),minstat=c("sumq","rcv")) {
    # Check arguments
    ...
    message("Calculating distances from Y=X...")
    ...
    if (multicore) {
        message("Parallel calculation of distances from Y=X...")
    ...
    if (is.element(cut.type,c("mad","qn","sn"))) {
        ...
        tmp <- apply(Xn,2,median,na.rm=TRUE)
        attr(Xn,"ri.info") <- list(ri.set=i.set,ri.all=i.all,metric=NULL,
            imetric=NULL,rcv=NULL,ircv=NULL,w.metric=NULL,w.imetric=NULL,
            w.rcv=rcv.mad(tmp),w.ircv=NULL)
        #attr(Xn,"metric") <- NULL
    ...
    for (b in 1:dim(M1)[[2]]) {
        ...
        for (i in 1:(steps + 1)) {
            ...
            if (aCurve[idxL] < aCurve[idxR])
                adjust[i] <- 
                    (((mCurve[idxR]-mCurve[idxL])/(aCurve[idxR]-aCurve[idxL]))
                        *(position[i]-aCurve[idxL])+mCurve[idxL])
        }
        # Apply lookup to data. Can be applied to transformed or untransformed 
        # data.
...
# Xo       : The original matrix, corrected for zeros, to be used for iterative 
#            fitting...
...
# set.only : return only the new set or also re-normalize? In the second case, 
#            a newly normalized matrix will be returned with attribute, the new 
#            ri.set
# opt      : optimize the final selection based on the variability of the 
#            normalized matrix, the standards or both
# caller   : NULL or grsn to call the GRSN algorithm own smoothing
# ... : further arguments to i.correct
# Returns a new set based on the minimization of the normalized matrix 
# variability
iterative.select <- function(Xo,ref,f,scal="mad",
    output=c("set","list","matrix"),multicore=FALSE,opt="data",coverage="none",
    minmax=c(1,length(f)),minstat="sumq",...) {
    require(utils)
    ...
    if (is.null(attr(Xo,"ri.info"))) {
        if (missing(f))
            stop("Input matrix does not have an \"ri.set\" attribute! Input f ",
                "must be provided!")
    }
    else
    ...
    if (multicore) {
        ...
        message("Parallel iterative selection... Please wait...")
        switch(scal,
            mad = {
                if (caller=="grsn")
    ...
    switch(output,
        set = { return(f.new) },
        list = { 
            return(list(ri.set=f.new,ri.all=f,metric=m,imetric=im,rcv=rcv,
                ircv=ircv,w.metric=m.f,w.imetric=im.f,w.rcv=rcv.f,
                w.ircv=ircv.f))
        },
        matrix = {
            Yn <- i.correct(Xn,ref,iset=f.new,...);
            attr(Yn,"ri.info") <- list(ri.set=f.new,ri.all=f,metric=m,
                imetric=im,rcv=rcv,ircv=ircv,w.metric=m.f,
                w.imetric=im.f,w.rcv=rcv.f,w.ircv=ircv.f);
            return(Yn)
         }
    )
}
...
# ignore.ncol : Perform an exhaustive feature ranking according to appearances, 
#               taking or not into account the number of columns in PM matrix 
#               which tells how many "first best" features we have. If TRUE, 
#               then it will return more than ncol(PM) features which express an
#               ordering of all the features that appear regardless of the 
#               "first best". FALSE will return an ordering of "first best"
#               Defaults to FALSE.
position.order <- function(PM,ignore.ncol=FALSE) {
    ...
    while (!all(visited) && vis.new!=vis.old) {
        ...
        for (j in 1:ncol(PM)) {
            ...
            # If there is nothing to the current j, advance
            if (all(visited[tmp.fet]))
                next
            
            # We may have a clear winner a but it requires more checking
            if (length(tmp.u)>1) { 
                ...
                if (length(which(tmp.app==max(tmp.u)))==1) {
                    ...
                    lucky <- FALSE
                # More ties or the clear winner was visited... we have something
                # like 2 2 1 1 1 1 ... Complicated...
                if (!lucky) {
                    # unique puts in order of appearance, so decrasing after sort
                    for (k in tmp.u) {
                        cf <- which(tmp.app==k)
