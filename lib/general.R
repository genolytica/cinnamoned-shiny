...
remove.zeros <- function(x,strategy=c("constant","offset","minpos",
    "minpos.noise","none"),off.set=1) {
    strategy <- strategy[1]
    switch(strategy,
        constant = { x[x<=0] <- 1 },
...
construct.peaktable <- function(xs,n.class,filter.n=0,digits=6,stats.out=TRUE) {
    ...
    if (class(xs)=="xcmsSet") {
        ...
        peaks.data <- split.peaktable(xs.table,n.class=n.class,
            filter.n=filter.n,digits=digits)
    }
        ...
        peaks.data <- split.peaktable(xs,n.class=n.class,filter.n=filter.n,
            digits=digits)

    return(peaks.data)
}

# x : a table from xcms peakTable, xsAnnotate getPeaklist or data.frame from 
#     annotatePeaks functions
split.peaktable <- function(x,n.class,filter.n=0,digits=6,stats.out=TRUE) {
    ...
    stats <- cbind(x$mz,x$rt,x$mzmin,x$mzmax,x$rtmin,x$rtmax,x$npeaks,
        class.data)

    cols <- c("isotopes","adduct","pcgroup","real.mass","prop.formula",
        "theor.mass","delta.ppm")
    ...
    colnames(stats)[1:7] <- 
        c("mz","rt","mzmin","mzmax","rtmin","rtmax","n.peaks")
    if (!is.null(ann))
        rownames(ann) <- rnames
...
match.mz <- function(mznew,mzref,tol=0.01) {
    m <- length(mzref)
    n <- length(mznew)
    match.ref <- list(mz.ref=numeric(m),mz.new=numeric(m),mz.diff=numeric(n),
        ref.idx=numeric(n),new.idx=NULL,pct=NULL)
    for (i in 1:n) {
        ...
        if (d<=tol) {
            ...
            #match.ref$ref.idx[i] <- match.ref$mz.new[i] <- 
            #   match.ref$mz.diff[i] <- NA
    }
    ...
    while (!all(mdev<tol) && its<=it) {
        ...
        for (i in unique(km$cluster)) {
            if (median(abs(rtnew[km$cluster==i]-rtref[km$cluster==i])) < 1e-6) {
                ...
                r <- rt.smooth(rtnew[km$cluster==i],rtref[km$cluster==i],
                    span=span,fail=fail[km$cluster==i])
                #rtcor[km$cluster==i] <- r$rtcor
                rtcor[km$cluster==i] <- rtnew[km$cluster==i] - r$rtdevmo
                #cldev[i] <- abs(median(r$rtcor-rtref[km$cluster==i]))
                cldev[i] <- 
                    abs(median(rtcor[km$cluster==i]-rtref[km$cluster==i]))
                curv[[i]]$x <- r$rtdevc$x
                curv[[i]]$y <- r$rtdevc$y
            }
        }
        ...
        if (length(naidx)>0) {
            rtdevtot[naidx] <- suppressWarnings(approx(
                na.omit(data.frame(rtcor,rtdevtot)),xout=rtcor[naidx],rule=2)$y)
...
rt.smooth <- function(rtnew,rtref,span,fail=NULL,corrfac=2) {
    ...
    # This is not likely to happen anymore after singleton removal
    if (length(rtdev)<2) { 
        warning("Cannot correct with one point! Global median correction will ",
            "be applied. Consider also lowering the number of iterations...",
            call.=FALSE)
    ...
    {
        lc <- loess(y ~ x,data.frame(x=x,y=y),span=span,degree=1,
            family="symmetric")
        ...
        if (length(naidx)>0)
            rtdevmo[naidx] <- suppressWarnings(approx(
                na.omit(data.frame(rtnew,rtdevmo)),xout=rtnew[naidx],rule=2)$y)
    }
...
is.intcor <- function(new,ref,iset,method=c("loess","rlm","lm"),span=0.75,
    adapt.factor=length(iset),corrfac=2,maxit=100) {
...
int.cor.loess <- function(x,ref,iset,span=0.25,corrfac=2,extrap=TRUE) {
    ...
    if (length(naidx)>0)
        pred[naidx] <- suppressWarnings(approx(na.omit(data.frame(ref,pred)),
            xout=pred[naidx],rule=2)$y)
    return((M-pred)+ref)
}
...
int.cor.rlm <- function(x,ref,iset,maxit=100) {
    ...
    fit <- rlm(y ~ x, data=data.frame(x=ref[iset],y=x[iset]-ref[iset]),
        maxit=maxit)
    #fit <- rlm(y ~ x,data=data.frame(
    #   x=(ref[iset]+x[iset])/2,y=x[iset]-ref[iset]),maxit=maxit)
    sig <- ifelse(fit$coefficients[2]>0,"+","")
    cat("\n","Intensity Normalization Curve: y = ",fit$coefficients[1],
        sig,fit$coefficients[2],"x","\n",sep="")
    ...
    # Because we are in the log scale, x-f is the same as dividing the original 
    # intensities by a factor f or multiplying by 1/f
}
...
int.cor.lm <- function(x,ref,iset,maxit=100) {
    ...
    cat("\n","Intensity Normalization Curve: y = ",fit$coef[1],
        sig,fit$coef[2],"x","\n",sep="")
    #return(x-fit$coef[2])
...
is.retcor.old <- function(rtnew,rtref,iset,span=0.25,extrap=NA,method="spline",
    corrfac=2) {
    rtdev <- rtnew-rtref
    y <- rtdev[iset]
    x <- rtref[iset]
...
is.intcor.old <- function(intnew,intref,iset,method="loess",span=0.75,it=2,
    fail=intref,corrfac=2,plotd=FALSE,...) {
    its <- 1
    ...
    while (its<=it) {
        ...
        for (i in unique(km$cluster))
        {
            r <- int.smooth.old(intnew[km$cluster==i],intref[km$cluster==i],
                span=span,fail=fail[km$cluster==i])
...
int.smooth.old <- function(intnew,intref,span,fail=NULL,corrfac=2) {
    ...
    if (length(m)<2) {
        ...
        if (!is.null(fail))
            ...
            stop("\nCannot correct with only one pair of points... Please ",
                "provide the \"fail\" option...",call.=FALSE)
    }
    else if (length(m)<8) {
        warning(paste("Not enough points (",length(m),") for a local ",
            "smoother... Will use a linear",sep=""),call.=FALSE)
        ...
        lc <- loess(m ~ a,data.frame(a=a,m=m),span=span,degree=1,
            family="symmetric")
        ...
        if (length(naidx)>0)
            mmo[naidx] <- suppressWarnings(approx(na.omit(data.frame(a,mmo)),
                xout=a[naidx],rule=2)$y)
    }
...
int.cor.rlm.old <- function(x,ref,iset,maxit=100) {
    if (!require(MASS))
        stop("R package MASS is required!")
    fit <- rlm(yy ~ xx, data=data.frame(xx=(ref[iset]+x[iset])/2,
        yy=x[iset]-ref[iset]),maxit=maxit)
